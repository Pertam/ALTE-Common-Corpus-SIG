...
lemmatisation, pilot dispersion-adjusted frequency sampling, sentence-level CEFR-derived communicative-function
...
> **provisional candidate material** that require expert review and corpus
...
### 4. Run the three tagging passes, then build a provisional combined dataset
...
# Pass 2 — informed validator pass reviewing Pass 1
python scripts/05b_run_pass2.py \
  --pass1    sample_data/pass1_en.csv \
  --taxonomy taxonomy/taxonomy_en.csv \
  --output   sample_data/pass2_en.csv

# Pass 3 — automated resolution pass (run where Pass 1 and Pass 2 disagree or confidence is low)
...
| **Pass 2** | Informed validator pass: reviews Pass 1, accepts, changes, or flags it as uncertain. It is not independent blind double coding. |
| **Pass 3** | Automated resolution pass for disagreements and low-confidence cases. It is not human adjudication and produces provisional candidate output. |
