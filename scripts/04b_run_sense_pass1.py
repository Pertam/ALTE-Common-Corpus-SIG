#!/usr/bin/env python3
"""Stage 04b: first independent sentence-level lexical-sense assignment."""
...
Use OTHER only for a clear missing sense and UNCLEAR only when context is insufficient.
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run independent lexical-sense Pass 1.")
    parser.add_argument("--samples", required=True); parser.add_argument("--inventory", required=True)
    parser.add_argument("--output", required=True); parser.add_argument("--model", default=default_model("sense_pass1"))
    ...
    for _, series in data.iterrows():
        ...
        print(f"Sense Pass 1 {row['row_id']}: {result['sense_id']}")
